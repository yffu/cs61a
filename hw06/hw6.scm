...
; c. Modify apply-generic so that it doesn't try coercion if the two arguments have the same type. added if statement after testing for type-tags length, to test if both tags were the same. If so, return error.x1
