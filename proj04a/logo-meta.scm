...
(define (logo-type val)
  (let ((line (make-line-obj val)))
    (define (helper next-token)
      (begin (
      (cond ((eq? next-token left-paren-symbol)
	     (begin (ask line 'put-back next-token)
		    (let ((evaled-exp (logo-eval line
	    ((eq? next-token ")") (display "]"))
	    (else (begin (display next-token)
			 (display " ")))))
	     (if (ask line 'empty?)
		 '=no-value=
		 (helper (ask line 'next)))))
    (helper (ask line 'next))))
