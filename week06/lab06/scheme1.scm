...
(define (substitute exp params args bound)
  (cond ((constant? exp) exp) ; trivial use of lambda
	((symbol? exp)
	 (if (memq exp bound) ; memq looks through the list, and if symbol is not in the list, it returns false. otherwise retuns sublist of the list beginning with the symbol.
	     exp
